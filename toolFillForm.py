...
from defineVariable import XPATH_NEXTBUTTON, XPATH_SUBMITBUTTON, XPATH_DATE_REPORT, XPATH_DROPDOWN_CENTER_CODE, XPATH_DROPDOWN_REPORT_CODE

from defineVariable import  formb4b9

from defineVariable import TEACHER_NAME, TEACHER_CODE, REPORT_CODE, DAY_REPORT
...
def selectReportCode(report_code):
    dropdown = driver.find_element(By.XPATH, XPATH_DROPDOWN_REPORT_CODE)
    dropdown.click()
    sleep(2)
    option_xpath = f'/html/body/div/div[2]/form/div[2]/div/div[2]/div[7]/div/div/div[2]/div/div[2]/div[{report_code}]'
    option = driver.find_element(By.XPATH, option_xpath)
    option.click()
    sleep(1)

def selectDate(day_report):
    date_field = driver.find_element(By.XPATH, XPATH_DATE_REPORT)
...
def autofillingForm1():
    ...
    for index, row in enumerate(raw_data, start=0):     
        ...
        selectReportCode(REPORT_CODE)
        selectDate(DAY_REPORT)
